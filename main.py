...
def main():
    ...
    df = preprocessing.make_robust_scaler(df=df)
    plotting.plot_amount_and_time_distribution(
    ...
    df = preprocessing.drop_unnecessary_columns(df=df)
    (
        original_Xtrain,
        original_Xtest,
        original_ytrain,
    ...
    normal_distributed_df, df_new = preprocessing.make_sub_sample_data_frame(df=df)
    preprocessing.check_target_distribution_sample_data_frame(df_new=df_new)
    plotting.plot_heat_map_for_data(
        df=df, df_new=df_new, output_path=settings.OUT_PUT_PATH
    )
    plotting.plot_box_plot_neg_corr(df_new=df_new, output_path=settings.OUT_PUT_PATH)

    plotting.plot_box_plot_pos_corr(df_new=df_new, output_path=settings.OUT_PUT_PATH)

    plotting.plot_distribution_neg_corr(
        df_new=df_new, output_path=settings.OUT_PUT_PATH
    )
    df_new, outliers_to_remove_V14 = preprocessing.make_outlier_removal(
        df_new=df_new, outlier_thre=settings.OUTLIER_THRE, column_name="V14"
    )
    df_new, outliers_to_remove_V12 = preprocessing.make_outlier_removal(
        df_new=df_new, outlier_thre=settings.OUTLIER_THRE, column_name="V12"
    )

    df_new, outliers_to_remove_V10 = preprocessing.make_outlier_removal(
        df_new=df_new, outlier_thre=settings.OUTLIER_THRE, column_name="V10"
    )
    ...
    X_reduced_tsne, y = dim_reduct.make_tsne_dim_reductor(
        df_new=df_new, n_components=settings.NUM_COMPONENTS
    )
